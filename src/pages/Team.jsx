...
const Team = () => {
  ...
  const organizeTeamMembers = (members) => {
    if (!members || members.length === 0) return { core: [], committee: [] };

    const coreRoles = ['President', 'Vice President', 'Secretary', 'Technical Head', 'Project & Innovation Head', 'Creative Head'];
    const committeeRoles = ['Technical Member', 'Marketing Member', 'Creative Member', 'Event Member', 'Documentation'];
  ...
  const { core, committee } = organizeTeamMembers(currentTeamData);
  const hasTeamData = currentTeamData.length > 0;
  ...
  const renderMemberCard = (member, index, delay = 0) => {
    ...
    >
      <div className="relative mb-4 flex-shrink-0">
        <div className="w-24 h-24 bg-gradient-to-r from-sky-500 to-blue-700 rounded-full mx-auto flex items-center justify-center overflow-hidden">
       ...
       <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-full flex justify-center">
          <span className="bg-gradient-to-r from-sky-500 to-blue-700 text-white px-2 py-1 rounded-full text-xs font-semibold whitespace-nowrap min-w-max">
            {member.role}
          </span>
        </div>
      </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
                  ...
                  {years.map((year) => (
                    <motion.button
                      ...
                      className={`px-4 py-2 rounded-full font-medium text-xs transition-all duration-300 ${
                        selectedYear === year
                          ? 'bg-gradient-to-r from-sky-500 to-blue-700 text-white shadow-md'
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400 hidden sm:block">
                Viewing: <span className="font-semibold text-sky-600 dark:text-sky-400">{selectedYear}</span>
              </div>
            </motion.div>
          </div>
        </div>
                ...
                {core.length > 0 && (
                  ...
                  >
                    <div className="text-center mb-12">
                      ...
                      <div className="w-24 h-1 bg-gradient-to-r from-sky-400 to-yellow-400 mx-auto rounded-full"></div>
                    </div>
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {years.filter(year => year !== selectedYear && teamData[year] && teamData[year].length > 0).map((year) => (
                    <motion.button
                      ...
                      className="px-4 py-2 bg-sky-100 hover:bg-sky-200 dark:bg-sky-900 dark:hover:bg-sky-800 text-sky-700 dark:text-sky-300 rounded-full text-sm font-medium transition-colors"
  ...
  <section className="py-20 bg-gradient-to-r from-sky-500 to-blue-400">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <h2 className="text-4xl md:text-5xl font-bold bg-gradient-to-r text-white bg-clip-text text-transparent mb-6 ">Want to Join Our Team?</h2>
              <p className="text-xl text-white mb-8 max-w-2xl mx-auto">We're always looking for passionate individuals to join our growing community</p>
              <motion.button 
                onClick={applynow}
                className="bg-white text-sky-600 px-8 py-4 rounded-full font-semibold text-lg hover:bg-gray-100 transition-colors shadow-lg"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                Apply Now
            ...
            >
              ...
              <div className="p-6 text-center">
                ...
                <div className="flex justify-center mb-3">
                  <span className="inline-block bg-gradient-to-r from-sky-500 to-blue-700 text-white px-3 py-1 rounded-full text-sm font-medium">
                    {lightboxImage.role}
                  </span>
                </div>
